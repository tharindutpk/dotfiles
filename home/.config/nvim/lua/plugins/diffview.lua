-- No plenary. diffview dropped that dependency; the only thing in this config
-- that still wanted it was typescript-tools, which vtsls replaced.
vim.pack.add({
  { src = "https://github.com/sindrets/diffview.nvim" },
}, { confirm = false })

-- diffview already defers its own heavy modules -- its plugin/ file registers
-- the commands and only pulls the rest in on first use -- so setup() runs on
-- the schedule tick rather than from a keymap. That way `:DiffviewOpen` typed
-- by hand behaves exactly like the mappings below.
